...
#    if 'ref' in attrs:
#        if caption != '': caption += ' '
#        caption += '<a href={ref}>Image source</a>'.format(ref=attrs.pop('ref'))
...
ReSourcedFig = re.compile("""(?P<class>\S.*\s+)?(?P<src>(?:https?:\/\/|\/|\S+\/)\S+)(?:\s+(?P<width>\d+))?(?:\s+(?P<height>\d+))?\s+(?P<quote>"|')(?P<alt>[^'"]+)?(?P=quote)(?:\s+(?P=quote)(?P<caption>[^"']+)?(?P=quote)(?:\s+(?P<ref>(?:https?:\/\/|\/|\S+\/)\S+)(?:\s+(?P<reftext>.+))?)?)?\s*""")
...
def sourced_fig(preprocessor, tag, markup):
    ...
    if 'ref' in attrs:
        if caption != '': caption += ' '
        caption += '<a href={ref}>{reftext}</a>'.format(ref=attrs.pop('ref'), reftext=attrs.pop('reftext', 'Image source'))
    html = '\t<p class="caption">{caption}</p>\n</div>\n'.format(caption=caption)
